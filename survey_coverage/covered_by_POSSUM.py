...
def tile_check(candidate,tile_sheet):
	"""Given candidate coordinates (SkyCoord object) and the Astropy Table with
	information on the tile status, figure out which survey tile the source
	belongs to and report the status of that tile."""

	tile_num = hp.ang2pix(32,candidate.ra.deg,candidate.dec.deg,lonlat=True)
	tile_row = tile_sheet[tile_sheet['tile_id']==tile_num]
	if len(tile_row) == 0:
		print("No match with planned tile; is target outside the survey area?\n")
		return
    
	output=f"Belongs to tile {tile_num}. "
	if ~tile_row['1d_pipeline_main'].mask[0]:
		output+=f"Tile was processed through 1D pipeline on {tile_row['1d_pipeline_main']}. "
	elif ~tile_row['aus_src'].mask[0]:
		output+=f"Tile was generated by AusSRC pipeline on {tile_row['aus_src']}. "
	if ~tile_row['3d_pipeline'].mask[0]:
		output+=f"Tile was processed through 3D pipeline on {tile_row['3d_pipeline']}. "
        
	print(output+'\n')
	return


def perform_cross_matching(candidate_coords, observed_positions, name_list,tile_sheet=None, separation_threshold=1.6):
	"""
	...
	for index, candidate in enumerate(candidate_coords):
		...
		if nearest_separation_deg < separation_threshold:
			...
			for i in range(3):
				output_string += f"Field = {nearest_SB_candidate[i]}, SBID = {nearest_SBID_candidate[i]}, Beam No. = {nearest_beams_candidate[i]}, Beam Sep. = {nearest_separations_deg_candidate[i]:.3f} degrees\n"
		...
		if 'OBSERVED' in output_string:
			...
			if tile_sheet is not None:
				tile_check(candidate,tile_sheet)
...
def main(args):
	...
	if band_used == '1':
		POSSUM_progress_csv_url = "https://docs.google.com/spreadsheets/d/1sWCtxSSzTwjYjhxr1_KVLWG2AnrHwSJf_RWQow7wbH0/gviz/tq?tqx=out:csv&sheet=Survey%20Fields%20-%20Band%201"
		...
		POSSUM_progress_csv_url = "https://docs.google.com/spreadsheets/d/1sWCtxSSzTwjYjhxr1_KVLWG2AnrHwSJf_RWQow7wbH0/gviz/tq?tqx=out:csv&sheet=Survey%20Fields%20-%20Band%202"
	...
	#Added by Cameron - load the relevant tile table for tile-position matching.
	tile_sheet=Table.read(f"https://docs.google.com/spreadsheets/d/1sWCtxSSzTwjYjhxr1_KVLWG2AnrHwSJf_RWQow7wbH0/gviz/tq?tqx=out:csv&sheet=Survey%20Tiles%20-%20Band%20{band_used}",format='ascii.csv')

	"""
	X-match candidate coords against beam and SBID. Return nearest 3 matches.
	"""
	observed_candidates = perform_cross_matching(candidate_coords, observed_POSSUM_survey_all_beam_locs, name_list, tile_sheet)
